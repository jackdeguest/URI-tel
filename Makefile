# This Makefile is for the URI::tel extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/URI/tel.pm]
#     AUTHOR => [q[Jacques Deguest <jack@deguest.jp>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     NAME => q[URI::tel]
#     PL_FILES => {  }
#     PREREQ_PM => {  }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/URI/tel.pm]
#     clean => { FILES=>q[URI-tel-*] }
...
EXE_EXT = 
FULL_AR = /usr/bin/ar
LD = cc -mmacosx-version-min=10.10.5
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) -u \
	  lib/URI/tel.pm $(INST_MAN3DIR)/URI::tel.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Implementation of rfc3966 for tel URI'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Jacques Deguest <jack@deguest.jp>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: URI-tel' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "Implementation of rfc3966 for tel URI",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Jacques Deguest <jack@deguest.jp>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "URI-tel",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="URI-tel" VERSION="0.2">' > URI-tel.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Implementation of rfc3966 for tel URI</ABSTRACT>' >> URI-tel.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Jacques Deguest &lt;jack@deguest.jp&gt;</AUTHOR>' >> URI-tel.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> URI-tel.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="darwin-thread-multi-2level-5.18" />' >> URI-tel.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> URI-tel.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> URI-tel.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> URI-tel.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/URI/tel.pm' 'blib/lib/URI/tel.pm' 
